...
The first event carries the full graph (same shape `/tree`/`/concept-map` return, plus the additive fields). Topology is built on the event loop via Motor; the numpy solve runs in a thread and pushes throttled frames through an `asyncio.Queue` (frames droppable/latest-wins, `final` never dropped). The frame cadence targets ~12 frames per solve — one every `ceil(iteration_budget/12)` solver iterations, but never faster than one per 80 ms — and the offer sequence always terminates `final`/`error` → `done`, Tier-0-tested in `backend/tests/test_layout_cadence.py`. A client disconnect cancels the solve. Positions are rounded to 1 dp, so the streamed `final` and the plain-GET response are byte-for-byte identical for the same request.
...
- `backend/tests/test_layout_cadence.py`: SPC-00021 §9 Tier-0 tests for the SSE frame-cadence policy — `_run_solver` driven directly with a stub job and a list-appending `offer` callback (real engine underneath, no IO). Asserts frames land only on stride multiples with the first stride hit always emitted, ≥ 80 ms between consecutive frames, ≤ 12 frames per solve, wire-rounded positions, and that `final`/`error` → `done` always terminate the sequence (including pre-set cancellation and an engine-rejected layout).
- `tests/fixtures/layout/final/` + `tests/integration/test_layout_characterization.py`: SPC-00021 §9 live characterization snapshots — each fixture stores a replayable layout request plus its settled positions; the live suite replays it and asserts node-id sets exactly, positions to `atol=0.5 px`, and `algo_version` exactly. Regenerate with `make capture-layout-snapshots` (requires `make run` + loaded data) after an `LAYOUT_ALGO_VERSION` bump or a Kaikki reload, and update the suite's pinned `EXPECTED_ALGO_VERSION` alongside an algo bump.
